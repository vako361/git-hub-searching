...
const Interface = ({ user, theme }: { user: gitHubUser | null , theme: boolean }) => {
 ...
 return(
    <div 
    className={theme ?'lg:w-[730px] :w-[350px] rounded-2xl h-[500px] bg-[#1E2A47] flex flex-row justify-between align-middle mb-[100px] ': 'rounded-2xl w-[730px] h-[500px] bg-[#FEFEFE] flex flex-row justify-between align-middle mb-[100px] '}>
    ...
    <section className="flex flex-col justify-start align-middle w-[500px] h-full pr-[20px]">
        ...
        <h2 className={`text-[26px] font-[700] ${theme ? 'text-[#fff]' : 'text-[#2B3442]'}`}>{user?.login}</h2>
        <p className="mt-[7px] text-[#697C9A] text-[15px]">{user?.created_at}</p>
        </div>
        <span className="text-[#0079FF] text-[16px] mt-1">@{user?.login}</span>
        <p className="text-[#4B6A9B] w-full mt-[25px] text-[15px] leading-[25px]">{user?.bio ? user.bio : 'This profile does not have bio'}</p>
        ...
        <section className={`flex gap-[90px]  justify-center align-middle mt-[32px] bg-${theme ? '[#ffffff20F]' : '[#F6F8FF]'} rounded-[10px]`}>
         ...
         {cardsData.map((item, index) => (
            <div key={index} className="flex flex-col h-[85px] mt-[15px]" >
              <span className={` text-[#4B6A9B] text-[13px] font-[400]`}>{item.title}</span>
              <span className={`${theme ? 'text-white' : 'text-[#2B3442]'} text-[22px] font-[700]`}>{item.value}</span>
            </div>
          ))}
